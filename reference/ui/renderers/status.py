...
def render_status_left(workspace_root: str, model: str | None, *, max_width: int | None = None) -> Text:
    model_text = model or "未配置模型"
    fixed_width = len("MiniAgent ·  · ") + len(model_text)
    cwd_width = None if max_width is None else max(1, max_width - fixed_width)
    workspace = _middle_ellipsize(workspace_root, cwd_width) if cwd_width is not None else workspace_root
    output = Text(style="ui.meta")
    output.append("MiniAgent", style="ui.label.agent")
    output.append(" · ")
...
def _middle_ellipsize(value: str, width: int) -> str:
    ...
    return _take_cells(value, left_width) + "…" + _take_cells(value, right_width, reverse=True)
